...
### Function definitions ###
    ...
    def HALS(data, shapes, activity, b_s, b_t, skip=[], check_skip=0):
        dims = data.shape
        data.shape = (dims[0], -1)
        S = zeros((L + 1, prod(dims[1:])))
        for ll in range(L):
            # print zeros((1,) + dims[1:]).shape, shapes[ll].shape, boxes[ll]
            S[ll] = RegionAdd(
        ...
        return residual, shapes, activity, b_s, b_t, skip, tsub
  ...
  #       return residual, shapes, activity, b_s, b_t, skip
    ...
    def HALS4activity(data, shapes, activity, b_s, b_t):
        dims = data.shape
        data.shape = (dims[0], -1)
        S = zeros((L + 1, prod(dims[1:])))
        for ll in range(L):
    ...
    def HALS4shape(data, shapes, activity, b_s, b_t):
        dims = data.shape
        data.shape = (dims[0], -1)
        S = zeros((L + 1, prod(dims[1:])))
        for ll in range(L):
...
### Get shape estimates on subset of data ###
    if iters0[0] > 0:
        ...
        for it in range(len(iters0)):
            for kk in range(iters0[it]):
                ...
                _, shapes, activity, b_s, b_t, skip, dt = HALS(
                    data0, shapes, activity, b_s, b_t, skip)  # replace data0 by residual for HALS using residual
                tsub += dt
            if it < len(iters0) - 1:
                mb = mbs[it + 1]
                data0 = data.reshape((-1, mb) + dims[1:]).mean(1)
                if adaptBias:
                    b_s *= b_t.mean()
                    b_t = ones(len(data0))
                activity = ones((L, len(data0))) * activity.mean(1).reshape(-1,1)
                residual, activity, b_t, dt = HALS4activity(data0, shapes, activity, b_s, b_t)
                tsub += dt
    ...
    # HALS
        ...
        activity = ones((L, dims[0])) * activity.mean(1).reshape(-1, 1)
        for _ in range(2):
            # replace data0 by residual for HALS using residual
            res, activity, b_t, dt = HALS4activity(dataSSS, shapes, activity, b_s, b_t)
            tsub += dt

        for ll in range(L):
            S = RegionAdd(
                zeros((1,) + dims0[1:]), shapes[ll].reshape(1, -1), boxes[ll]).ravel()
            S = repeat(repeat(S.reshape(dims0[1:]), 2, 0), 2, 1)
            boxes[ll] = GetBox(centers[ll], R, dims[1:])
            shapes[ll] = RegionCut(S.reshape((1,) + dims[1:]), boxes[ll])[0]
        b_s = repeat(repeat(b_s.reshape(dims0[1:]), 2, 0), 2, 1).ravel()

        for _ in range(2):
            residual, shapes, b_s, dt = HALS4shape(data, shapes, activity, b_s, b_t)
...
#### Main Loop ####
    ...
    for kk in range(iters):
        ...
        residual, shapes, activity, b_s, b_t, skip, dt = HALS(
            data, shapes, activity, b_s, b_t, skip, kk + 1)  # replace data0 by residual for HALS using residual
        # Recenter
        # if kk % 30 == 20:
        #     for ll in range(L):
...
plt.ylabel('MSE')
plt.xlim(0, 25)
plt.ylim(2.5e9, 2.75e9)
# plt.ylim(2.52e9, 2.55e9)
plt.show()
