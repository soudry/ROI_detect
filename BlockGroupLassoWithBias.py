...
from numpy import zeros, maximum, reshape, prod, std, sum, nan_to_num, \
    mean, max, sqrt, percentile, dot, outer
...
def gaussian_group_lasso(data, sig, lam=0.5, tol=1e-2, iters=30, NonNegative=False, TargetAreaRatio=[], verbose=False, adaptBias=False):
...
def fista(data, prox, Omega, A, lam, L, x0=None, tol=1e-8, iters=100, NonNegative=False, verbose=False, adaptBias=False):
    ...
    if adaptBias:
        v_t, v_s = greedyNNPCA(data, percentile(data, 30, 0).reshape(-1), 5)
        bias = outer(v_t, v_s).reshape(data.shape)
    else:
        bias = 0 * data
    ...
    for kk in range(iters):
        ...
        qk = yk - 2 / L * A(r + bias, do_transpose=True) + v
        if adaptBias and kk % 5 == 0:
            v_t, v_s = greedyNNPCA(data - r, v_s, 5)
            bias = outer(v_t, v_s).reshape(data.shape)
        xk = prox(qk, lam / L)
