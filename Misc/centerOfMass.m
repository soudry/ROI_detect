function [C, V] = centerOfMass(A,varargin)
% CENTEROFMASS finds the center and variance of mass of the N-dimensional input array
%
...
%
%   UPDATES
%       2014/06/18 - daniel soudry - v1.1
%         transposed output and added variance output
%
%
...
if M==0
    ...
    for ii = 1:nd
        ...
        C(ii) = sum(ind(:).*A(:))./M;
        ind = repmat(reshape((1:sz(ii)).^2,shp),rep);
        V(ii) = sum(ind(:).*A(:))./M - C(ii)^2;       
    end
end
